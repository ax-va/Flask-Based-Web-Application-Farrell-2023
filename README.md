...
The first part, `xyz`, corresponds to the name of some 'xyz.py' script that is an entry point. `xyz` is either `blog` or `app`. The second part, `:app`, refers to the Flask application instance `app` created in some 'xyz.py':
```python
app: Flask = ...
```
